...
- **`pickLocalAngle({ topicIsGeneral, seed })`** — returns a local anchor string or `null`. General topics → **always null**. Otherwise an anchor appears **~40% of the time** (`LOCAL_INCLUSION_RATE`) — a deliberate **40 local / 60 not-local** split, decided deterministically from the seed; towns are biased **~2:1** over region phrases; the town rotates by seed. US-coded verticals (legal) never carry a Hampshire anchor (the pool is UK).
...
- **Shorts/Reels** (`generate-shorts.ts`): `buildPrompt(title, content, sourceUrl)` injects the vertical + local brief so the on-screen text and voiceover speak to the chosen vertical's reader (not generic "service businesses"), with **at most one** spoken local aside when an anchor is present. `generatePlatformMeta(script, platform, { plan, salt })` then localises deterministically: YouTube weaves the town into the title (only if it still fits 60 chars) + a description location line; Instagram/Facebook captions get a local mention and a **small rotating local hashtag set** (IG stays within its 5-tag cap, ~2 local tags). **LinkedIn is kept deliberately light** — a more national, professional audience — so it gets at most **one** local hashtag plus a light local mention (same 40/60 asset ratio, just a lighter touch; never a hyperlocal tag block). No anchor → clean copy, no place names, no local tags.
